...
class SupervisorActorTaskOneAndTaskTwo extends Actor{
  ...
  override def receive: Receive = {
    ...
    case CreateChild(name) =>{
            val child = context.actorOf(ChildActorTaskOneAndTaskTwo.props, name = s"$name") // Getting context of the child which actor which is to be called (Step - 5)
            child ! RecordNumberOfHopsTravelled(numberOfHopsTravelled, name) //  // Calling the child actor (Step - 6)
            increaseHopsValueByOne(child)// Increment the value of hops (Step - 8)
    }
  ...
  def increaseHopsValueByOne(child: ActorRef): Unit = {
      ...
      numberOfHopsTravelled = result.count // Reassign the hops value to the by current count (Step - 11)
  }
